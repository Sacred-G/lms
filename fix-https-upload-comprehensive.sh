#!/bin/bash
# This script provides a comprehensive fix for HTTPS Nginx upload size limit issues
# For Windows users: Run this script using Git Bash or WSL

# VPS SSH details
VPS_SSH="root@147.93.41.71"
DOMAIN="vps.sbouldin.com"

echo "Starting comprehensive fix for HTTPS Nginx upload size limit on VPS..."

# SSH into VPS to check and fix the Nginx configuration
echo "Checking and fixing Nginx configuration on VPS..."
ssh ${VPS_SSH} << 'EOF'
  echo "Creating backup of all Nginx configuration files..."
  
  # Find all Nginx configuration files
  NGINX_CONF_FILES=$(find /etc/nginx -type f -name "*.conf" 2>/dev/null)
  NGINX_SITE_FILES=$(find /etc/nginx/sites-available -type f 2>/dev/null)
  NGINX_ENABLED_FILES=$(find /etc/nginx/sites-enabled -type f 2>/dev/null)
  
  # Combine all unique files
  ALL_NGINX_FILES=$(echo "$NGINX_CONF_FILES $NGINX_SITE_FILES $NGINX_ENABLED_FILES" | tr ' ' '\n' | sort | uniq)
  
  # Create backups of all files
  for FILE in $ALL_NGINX_FILES; do
    if [ -f "$FILE" ]; then
      cp "$FILE" "${FILE}.bak"
      echo "Backed up $FILE"
    fi
  done
  
  # Add client_max_body_size to http block in main nginx.conf
  MAIN_CONF="/etc/nginx/nginx.conf"
  if [ -f "$MAIN_CONF" ]; then
    echo "Checking main Nginx configuration at $MAIN_CONF..."
    
    # Check if client_max_body_size is already set in the http block
    if grep -q "client_max_body_size" "$MAIN_CONF"; then
      echo "client_max_body_size is already set in main configuration."
      echo "Current setting:"
      grep "client_max_body_size" "$MAIN_CONF"
      
      # Update the client_max_body_size to 100M
      echo "Updating client_max_body_size to 100M in main configuration..."
      sed -i 's/client_max_body_size [0-9]*[kKmMgG]\?;/client_max_body_size 100M;/g' "$MAIN_CONF"
    else
      echo "client_max_body_size is not set in main configuration."
      echo "Adding client_max_body_size 100M to http block..."
      
      # Add client_max_body_size to the http block
      sed -i '/http {/a \    client_max_body_size 100M;' "$MAIN_CONF"
    fi
  fi
  
  # Process all server blocks in all configuration files
  for FILE in $ALL_NGINX_FILES; do
    if [ -f "$FILE" ]; then
      echo "Processing $FILE..."
      
      # Check if this file contains server blocks
      if grep -q "server {" "$FILE"; then
        echo "Found server blocks in $FILE"
        
        # Check if client_max_body_size is already set in any server block
        if grep -q "client_max_body_size" "$FILE"; then
          echo "client_max_body_size is already set in $FILE."
          echo "Current settings:"
          grep "client_max_body_size" "$FILE"
          
          # Update all client_max_body_size directives to 100M
          echo "Updating all client_max_body_size directives to 100M..."
          sed -i 's/client_max_body_size [0-9]*[kKmMgG]\?;/client_max_body_size 100M;/g' "$FILE"
        else
          echo "client_max_body_size is not set in $FILE."
          echo "Adding client_max_body_size 100M to all server blocks..."
          
          # Add client_max_body_size to all server blocks
          # This is a bit tricky with sed, so we'll use a temporary file
          cp "$FILE" "$FILE.tmp"
          awk '/server {/ {print; print "    client_max_body_size 100M;"; next} {print}' "$FILE.tmp" > "$FILE"
          rm "$FILE.tmp"
          
          # Verify the changes
          if grep -q "client_max_body_size" "$FILE"; then
            echo "Successfully added client_max_body_size directive to $FILE"
          else
            echo "Failed to add client_max_body_size directive to $FILE"
          fi
        fi
      fi
    fi
  done
  
  # Check for proxy configurations that might be limiting request size
  echo "Checking for proxy configurations..."
  PROXY_CONF_FILES=$(grep -l "proxy_pass" $ALL_NGINX_FILES 2>/dev/null)
  
  for FILE in $PROXY_CONF_FILES; do
    if [ -f "$FILE" ]; then
      echo "Found proxy configuration in $FILE"
      
      # Check if proxy_request_buffering is set to off
      if ! grep -q "proxy_request_buffering off" "$FILE"; then
        echo "Adding proxy_request_buffering off to $FILE..."
        
        # Find proxy_pass lines and add proxy_request_buffering after them
        sed -i '/proxy_pass/a \        proxy_request_buffering off;' "$FILE"
      fi
      
      # Check if proxy_max_temp_file_size is set
      if ! grep -q "proxy_max_temp_file_size" "$FILE"; then
        echo "Adding proxy_max_temp_file_size 0 to $FILE..."
        
        # Find proxy_pass lines and add proxy_max_temp_file_size after them
        sed -i '/proxy_pass/a \        proxy_max_temp_file_size 0;' "$FILE"
      fi
    fi
  done
  
  # Test Nginx configuration
  echo "Testing Nginx configuration..."
  nginx -t
  
  # Reload Nginx if configuration test passes
  if [ $? -eq 0 ]; then
    echo "Nginx configuration test passed. Reloading Nginx..."
    systemctl reload nginx
    echo "Nginx reloaded successfully."
  else
    echo "Nginx configuration test failed. Restoring backups..."
    
    # Restore backups if the test fails
    for FILE in $ALL_NGINX_FILES; do
      if [ -f "${FILE}.bak" ]; then
        cp "${FILE}.bak" "$FILE"
        echo "Restored $FILE from backup"
      fi
    done
    
    # Test again after restoration
    echo "Testing Nginx configuration after restoration..."
    nginx -t
    
    if [ $? -eq 0 ]; then
      echo "Nginx configuration test passed after restoration. Reloading Nginx..."
      systemctl reload nginx
      echo "Nginx reloaded successfully."
    else
      echo "Nginx configuration test still failing. Manual intervention required."
      exit 1
    fi
  fi
  
  # Check if there's an Apache server running as well
  if command -v apache2 &> /dev/null; then
    echo "Apache server detected. Checking Apache configuration..."
    
    # Check if LimitRequestBody is set in Apache configuration
    APACHE_CONF="/etc/apache2/apache2.conf"
    if [ -f "$APACHE_CONF" ]; then
      if ! grep -q "LimitRequestBody" "$APACHE_CONF"; then
        echo "Adding LimitRequestBody directive to Apache configuration..."
        echo "LimitRequestBody 104857600" >> "$APACHE_CONF"
        
        # Restart Apache
        echo "Restarting Apache..."
        systemctl restart apache2
      else
        echo "LimitRequestBody is already set in Apache configuration."
        echo "Current setting:"
        grep "LimitRequestBody" "$APACHE_CONF"
      fi
    fi
  fi
  
  # Check for other potential issues
  echo "Checking for other potential issues..."
  
  # Check if uploads directory exists and has proper permissions
  UPLOADS_DIR="/home/lms/uploads"
  if [ -d "$UPLOADS_DIR" ]; then
    echo "Checking permissions on uploads directory..."
    chmod -R 755 "$UPLOADS_DIR"
    chown -R www-data:www-data "$UPLOADS_DIR"
    echo "Set permissions on uploads directory"
  else
    echo "Creating uploads directory..."
    mkdir -p "$UPLOADS_DIR"
    chmod -R 755 "$UPLOADS_DIR"
    chown -R www-data:www-data "$UPLOADS_DIR"
    echo "Created uploads directory with proper permissions"
  fi
  
  # Check if client/public/audio directory exists and has proper permissions
  AUDIO_DIR="/home/lms/client/public/audio"
  if [ -d "$AUDIO_DIR" ]; then
    echo "Checking permissions on audio directory..."
    chmod -R 755 "$AUDIO_DIR"
    chown -R www-data:www-data "$AUDIO_DIR"
    echo "Set permissions on audio directory"
  else
    echo "Creating audio directory..."
    mkdir -p "$AUDIO_DIR"
    chmod -R 755 "$AUDIO_DIR"
    chown -R www-data:www-data "$AUDIO_DIR"
    echo "Created audio directory with proper permissions"
  fi
  
  echo "Comprehensive HTTPS upload size limit fix completed on VPS!"
EOF

echo "Comprehensive fix for HTTPS upload size limit completed!"
echo "Your VPS should now allow uploading larger audio files (up to 100MB) over HTTPS."
echo "Try uploading audio files through the admin panel again."
